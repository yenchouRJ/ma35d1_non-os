 ...
 * @file     main.c
 *
 * @brief    FreeRTOS-SMP SSL client using LwIP + mbedTLS.
 *           Client IP address 192.168.1.3, connects to server at 192.168.1.2.
 *           Sends "nuvoton" and expects "Hello World!!" in response.
 *
...
int main(void)
{
    ...
    {
        TaskHandle_t xSslTaskHandle = NULL;
        xTaskCreate( vSslTask, "SslTask", SSL_THREAD_STACKSIZE, NULL, SSL_TASK_PRIORITY, &xSslTaskHandle );
        vTaskCoreAffinitySet( xSslTaskHandle, 0x01 );
    }
#else
